...
def lookup_shot_gained(distance, shot_type):
    if distance == 0:
        return 0
    ...
    if shot_type != 'green':
        try:
            type_index = type_indexes[shot_type]
        except KeyError:
            raise RuntimeError(f'Invalid shot type: {shot_type} is not in {type_indexes.keys()}')
    ...
    shots_gained = shots_table[distance_index][type_index]
    if not shots_gained:
        raise ValueError(f'Invalid shot type: {shot_type} and distance: {distance} combination')
...
def get_shot_gained(from_distance, from_type, to_distance, to_type, shots=1):
    ...
    if to_distance == 0:
        return round(start_shots_gained - shots, 2)
